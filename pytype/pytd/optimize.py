...
class FindCommonSuperClasses(visitors.Visitor):
  ...
  E.g., this changes
    def f(x: list | tuple, y: frozenset | set) -> int | float
  to
...
class AbsorbMutableParameters(visitors.Visitor):
  ...
  For example, this will change
    def f(x: list[int]):
      x = list[int | float]
  to
    def f(x: list[int] | list[int | float])
  .
...
def Optimize(
  ...
  Arguments:
    ...
    use_abcs: Use abstract base classes to represent unions like e.g. "float |
      int" as "Real".
