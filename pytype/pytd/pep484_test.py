...
class TestPEP484(parser_test.ParserTest):
  ...
  def test_convert_optional(self):
    t = pytd.GenericType(pytd.ExternalType("Optional", module="typing"),
                         (pytd.NamedType("str")))
    self.assertEquals(self.convert(t), "str or NoneType")
  ...
  def test_convert_union(self):
    ...
    self.assertEquals(self.convert(t), "str or float")
  ...
  def test_convert_any(self):
    ...
    self.assertEquals(self.convert(t), "?")

  def test_convert_anystr(self):
    t = pytd.ExternalType("AnyStr", module="typing")
    self.assertEquals(self.convert(t, python_version=(2, 7)), "str or unicode")
    t = pytd.ExternalType("AnyStr", module="typing")
    self.assertEquals(self.convert(t, python_version=(3, 4)), "bytes or str")
