...
def to_pytd_def(val: abstract.BaseValue) -> pytd.Node:
  ...
  if isinstance(val, abstract.BaseClass):
    return _class_to_pytd_def(val)
  elif isinstance(val, abstract.InterpreterFunction):
    return _function_to_pytd_def(val)
  else:
    raise NotImplementedError(
        f'to_pytd_def() not implemented for {val.__class__.__name__}: {val}')


def _class_to_pytd_def(val: abstract.BaseClass) -> pytd.Class:
  ...
  for member_name, member_val in val.members.items():
    ...
    try:
      ...
      member_type = to_pytd_type(member_val)
    if isinstance(member_type, pytd.Function):
      ...
      constants.append(pytd.Constant(name=member_name, type=class_member_type))
...
def _function_to_pytd_def(val: abstract.InterpreterFunction) -> pytd.Function:
  """Converts an abstract function to a pytd.Function."""

  def get_pytd(param_name):
    if param_name in sig.annotations:
      return to_pytd_instance_type(sig.annotations[param_name])
    else:
  ...
  for sig in val.signatures:
    ...
    if 'return' in sig.annotations:
      ret_type = to_pytd_instance_type(sig.annotations['return'])
    else:
      func_frame = val.call_with_mapped_args(sig.make_fake_args())
      ret_type = to_pytd_type(func_frame.get_return_value())
...
def to_pytd_type(val: abstract.BaseValue) -> pytd.Type:
  ...
  Args:
    val: The abstract value.
  """
  if val is abstract.ANY:
    return pytd.AnythingType()
  elif isinstance(val, abstract.Union):
    return pytd_utils.JoinTypes(to_pytd_type(v) for v in val.options)
    ...
    if fixed_length_posargs_only:
      pytd_sig, = to_pytd_def(val).signatures
      ...
      ret = abstract.join_values([frame.get_return_value()
                                  for frame in val.analyze()])
      return pytd.GenericType(
          base_type=pytd.NamedType('typing.Callable'),
          parameters=(pytd.AnythingType(), to_pytd_type(ret)),
      )
  else:
    raise NotImplementedError(
...
def to_pytd_instance_type(val: abstract.BaseValue) -> pytd.Type:
  ...
  if val is abstract.ANY:
    ...
    return pytd_utils.JoinTypes(to_pytd_instance_type(v) for v in val.options)
  elif isinstance(val, abstract.BaseClass):
