...
class ErrorTest(test_base.BaseTest):
  ...
  def test_pretty_print_wrong_args(self):
    ...
    self.assertErrorSequences(
        errors, {"e": ["a, b, c, d: int, ...", "a, b, c, d: str, ..."]}
    )
  ...
  def test_attribute_error(self):
    with test_utils.Tempdir() as d:
      ...
      if self.python_version in ((3, 10), (3, 12)):
  ...
  @test_utils.skipIfPy(
      (3, 10),
      (3, 12),
      reason="non-3.10/3.12: log one error for all bad options",
  )
...
class OperationsTest(test_base.BaseTest):
  ...
  def test_binary(self):
    ...
    """)
    self.assertErrorSequences(
        errors, {"e": ["**", "int", "str", "__pow__ on", "int"]}
    )
